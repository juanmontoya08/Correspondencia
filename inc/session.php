...
class Session {
	...
	public function isLogged() {
		...
		$prepared = $this->prepare("SELECT count(*) as c FROM cor_users WHERE username=? && password=?", 'isLogged()');
	...
	public function get_user_id() {
		...
		$prepared = $this->prepare("SELECT id FROM cor_users WHERE username=?", 'get_user_id()');
	...
	public function get_user_name_by_id($id) {
		$prepared = $this->prepare("SELECT username FROM cor_users WHERE id=?", 'get_user_name_by_id()');
	...
	public function get_radicado_by_id($id) {
		$prepared = $this->prepare("SELECT nRadicado FROM cor_correspondencia WHERE id=?", 'get_radicado_by_id()');
		$this->bind_param($prepared->bind_param('i', $id), 'get_radicado_by_id()');
		$this->execute($prepared, 'get_radicado_by_id()');
		
		$result = $prepared->get_result();
		$row = $result->fetch_object();
		
		return $row->nRadicado;
	}
	
	public function get_user_role() {
		$id = $this->get_user_id();
		
		$prepared = $this->prepare("SELECT role FROM cor_users WHERE id=?", 'get_user_role()');
